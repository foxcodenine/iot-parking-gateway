...
CREATE TABLE IF NOT EXISTS parking.lora_keepalive_logs (
    id SERIAL,
    ...
    time_sync_rand_byte SMALLINT, 
    time_sync_current_unix_time BIGINT DEFAULT NULL, 


    PRIMARY KEY (id, happened_at)
);

-- Convert 'lora_keepalive_logs' to a TimescaleDB hypertable for optimized performance.
SELECT create_hypertable('parking.lora_keepalive_logs', 'happened_at');

-- Indexes for improved query performance.
CREATE INDEX idx_lora_keepalive_logs_device_id ON parking.lora_keepalive_logs (device_id);
CREATE INDEX idx_lora_keepalive_logs_happened_at ON parking.lora_keepalive_logs (happened_at);
